window.addEventListener('load', () =&gt; {
  ...
  async function search (path) {
    ...
    $input.addEventListener('input', function () {
      ...
      if (this.value.trim().length &lt;= 0) return let count="0" perform local searching datas.foreach(function (data) { ismatch="true" if (!data.title || data.title.trim()="==" '') data.title } datatitle="data.title.trim().toLowerCase()" const datacontent="data.content" ? data.content.trim().replace( &lt;[^&gt;]+&gt;/g, '').toLowerCase() : ''
